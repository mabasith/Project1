...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #Set dimension of the filter.
    
    dim = 2 * (N + num_landmarks)
    
    #Make constaine in formation matrix and vectors.
    ...
    for k in range(len(data)):
        ...
        for i in range(len(measurement)):
            ...
            # m is the index of the landmark co-ordinate in the materix/vector
            m = 2 * (N + measurement[i][0])
            # Update the information on matrix/vector based on the measurement
            for b in range(2):
